class RadiationAPIService
  ...
  class << self
    def irradiance_between_datetimes(begin_datetime, end_datetime, household)
      ...
      @@irradiance_peaks_per_month[begin_datetime.month]
    end

    def irradiance_by_datetime(datetime)
      # Return irradiance value based on the date of the highest WH peak.
      ...
      # url = "https://developer.nrel.gov/api/solar/solar_resource/v1.format?api_key=kUJgs2bV92fhQLV1eMGw89wTzfyFMLPpZv0DBxAg&lat=#{@household.latitude}&lng=#{@household.longitude}";
      # response = HTTParty.get(url)
      @@irradiance_peaks_per_month[datetime.month]
    end
  end
end
